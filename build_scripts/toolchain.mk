...
$(TOOLCHAIN_PREFIX)/bin/i686-elf-ld: $(BINUTILS_SRC).tar.xz
	...
	cd $(BINUTILS_BUILD) && CFLAGS="-Wno-error -fpermissive" CXXFLAGS="-Wno-error -fpermissive" ASMFLAGS= CC= CXX= LD= ASM= LINKFLAGS= LIBS= ../binutils-$(BINUTILS_VERSION)/configure \
...
$(TOOLCHAIN_PREFIX)/bin/i686-elf-gcc: $(TOOLCHAIN_PREFIX)/bin/i686-elf-ld $(GCC_SRC).tar.xz
	...
	mkdir $(GCC_BUILD)
	cd $(GCC_BUILD) && CFLAGS="-Wno-error -fpermissive" CXXFLAGS="-Wno-error -fpermissive" ASMFLAGS= CC= CXX= LD= ASM= LINKFLAGS= LIBS= ../gcc-$(GCC_VERSION)/configure \
		--prefix="$(TOOLCHAIN_PREFIX)" 	\
		--target=$(TARGET)				\
		--disable-nls					\
